  ...
  bind(ev) {
    ...
    ev.on('messages.upsert', ({ messages }) => {
      for (const msg of messages) {
        ...
        const rawParticipant = msg.key.participant || rawJid;
        ...
        const senderKey = isParticipantLid && rawAlt && !rawAlt.endsWith('@lid') && !rawAlt.endsWith('@hosted.lid')
          ? jidNormalizedUser(rawAlt)
          : jidNormalizedUser(rawParticipant);

        const name = msg.pushName;

        if (name && senderKey && !senderKey.endsWith('@lid') && !senderKey.endsWith('@hosted.lid')) {
          if (!this.contacts[senderKey]) {
            this.contacts[senderKey] = { id: senderKey, name };
          } else if (this.contacts[senderKey].name !== name) {
            this.contacts[senderKey].name = name;
          }
        }
